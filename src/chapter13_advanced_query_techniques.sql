...
-- using subquery to identify rows to delete
CREATE TABLE us_counties_2019_top10
AS SELECT * FROM us_counties_pop_est_2019;
...
/*
in LATERAL clause table name can be omitted if it is the same as in FROM,
but every LATERAL must have reference in the end even if not used in SELECT
*/
...
ORDER BY t.id;
/*
 works as a for loop - for every row returned from SELECT, the subquery is
 executed
 */
...
SELECT state_name, count(*) AS counties_count
FROM large_counties
GROUP BY state_name
ORDER BY count(*) DESC;
...
-- WITH + JOIN
WITH
    counties (st, pop_est_2018)
    AS (SELECT state_name, sum(pop_est_2018)
        FROM us_counties_pop_est_2019
        GROUP BY state_name),
    establishments (st, establishment_count)
    AS (SELECT st, sum(establishments) AS establishments_count
        FROM cbp_naics_72_establishments
        GROUP BY st)
...
AS estabs_per_tousand
FROM counties JOIN establishments
ON counties.st = establishments.st
ORDER BY estabs_per_tousand DESC;

-- we do not need to define us_median_pop twice, WITH table can be referenced
WITH us_median
AS (SELECT percentile_cont(.5)
